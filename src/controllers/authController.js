...
async function register(req, res) {
  try {
    ...
    if (!name || !email || !password) {
      return res.status(400).json({ success: false, message: "Nama, email, dan password dibutuhkan." });
    }
...
async function login(req, res) {
  try {
    ...
    if (!email || !password) return res.status(400).json({ success: false, message: "Email dan password dibutuhkan." });
    ...
    const token = jwt.sign(payload, process.env.JWT_SECRET || "secretkey", { expiresIn: "30m" });

    return res.json({ success: true, data: { token, user: { id: user._id, name: user.name, email: user.email, role: user.role, avatar: user.avatar } } });
  } catch (err) {
    console.error(err);
